...
const TestConfig = {
    ...
    cryptoAddresses: [{
            ...
            currencyCode: "ZEC",
            address: "t1QvpnirY5E3zW2AzbfqF3JkbT9coTiaKkh"
        }, {
...
const HAVENO_WALLETS: Map<HavenoClient, moneroTs.MoneroWalletFull> = new Map<HavenoClient, moneroTs.MoneroWalletFull>();
...
test("Has a Monero wallet (Test, CI)", async () => {
  ...
  try {
    ...
    if (!err.message.includes("Failed to parse hex")) throw new Error("Unexpected error: " + err.message);
  }
...
test("Can schedule offers with locked funds (Test, CI)", async () => {
  ...
  assert.equal(await isMining(), false, "Cannot run test while mining is active");
    ...
    // start user3
    user3 = await initHaveno();
    const user3Wallet = await getWallet(user3);
...
test("Invalidates offers when reserved funds are spent (Test, CI)", async () => {
  ...
  try {
    ...
    // restore user1's wallet from seed to spend funds externally
    const user1Wallet = await getWallet(user1);
    ...
    // offer reserves funds
    expect(BigInt((await user1.getBalances()).getReservedOfferBalance())).toBeGreaterThan(0n);
    ...
    // spend offer's reserved outputs by sweeping restored wallet, which does not freeze them
    await user1Wallet.sync();
    const txs = await sweepOutputsIndividually(user1Wallet, false);
    if (!txs.length) throw new Error("No txs created to spend reserved outputs");
    for (const tx of txs) await monerod.submitTxHex(tx.getFullHex()!, true);
...
test("Can handle unexpected errors during trade initialization (Test)", async () => {
  ...
  try {
    ...
    // restore trader wallets from seed to spend funds externally
    const trader0Wallet = await getWallet(traders[0]);
    const trader1Wallet = await getWallet(traders[1]);
    ...
    let paymentAccount = await createPaymentAccount(traders[1], TestConfig.trade.assetCode!, TestConfig.trade.paymentMethodId!);
    const spendDelay = 100;
    wait(spendDelay).then(async function() {
      try {
        await trader1Wallet.sync();
        HavenoUtils.log(1, "Sweeping trade funds");
        await trader1Wallet.sweepUnlocked({address: await trader1Wallet.getPrimaryAddress(), relay: true});
      } catch (err: any) {
    ...
    try {
      ...
      if (err.message.includes("Should have failed")) throw new Error("Unexpected error: " + err.message);
    }
    ...
    wait(spendDelay).then(async function() {
      try {
        await trader0Wallet.sync();
        HavenoUtils.log(1, "Sweeping offer funds");
        await trader0Wallet.sweepUnlocked({address: await trader0Wallet.getPrimaryAddress(), relay: true});
        HavenoUtils.log(1, "Done sweeping offer funds");
      } catch (err: any) {
        console.log("Caught error sweeping funds!");
        console.log(err);
      }
    ...
    paymentAccount = await createPaymentAccount(traders[2], TestConfig.trade.assetCode!, TestConfig.trade.paymentMethodId!);
    try {
...
async function initHaveno(ctx?: HavenodContext): Promise<HavenoClient> {
  ...
  try {
    ...
    const cmd: string[] = [
      ...
      "--walletRpcBindPort", "" + await getAvailablePort(),
...
async function releaseHavenoClient(client: HavenoClient, deleteProcessAppDir?: boolean) {
  await closeHavenodWallet(client);
...
async function releaseHavenoProcess(havenod: HavenoClient, deleteAppDir?: boolean) {
  ...
  await closeHavenodWallet(havenod);
...
/**
 * Get an in-memory wallet restored from the given daemon's seed in order to test internal state.
 */
async function getWallet(havenod: HavenoClient): Promise<moneroTs.MoneroWalletFull> {
  if (!HAVENO_WALLETS.has(havenod)) {
    const wallet = await moneroTs.createWalletFull({
      networkType: TestConfig.networkType,
      seed: await havenod.getXmrSeed(),
      server: {uri: TestConfig.monerod.url, username: TestConfig.monerod.username, password: TestConfig.monerod.password}
    });
    await wallet.sync();
    await wallet.startSyncing(TestConfig.trade.walletSyncPeriodMs);
...
/**
 * Close the wallet restored from the given daemon's seed, if open.
 */
async function closeHavenodWallet(havenod: HavenoClient) {
  const wallet = HAVENO_WALLETS.get(havenod);
  if (!wallet) return;
  HAVENO_WALLETS.delete(havenod);
  await wallet.close();
}
...
/**
 * Get the number of outputs of the given amount which its haveno daemon may have frozen
 * for reserved offers, since wallets restored from seed cannot see frozen state.
 */
async function getNumReservedOutputs(wallet: moneroTs.MoneroWallet, amt: bigint): Promise<number> {
  for (const [havenod, havenoWallet] of HAVENO_WALLETS.entries()) {
    if (havenoWallet === wallet) return Number((BigInt((await havenod.getBalances()).getReservedOfferBalance()) + amt - 1n) / amt);
  }
  return 0;
}

/**
 * Sweep a wallet's unlocked outputs in individual txs, since large sweep txs can fail
 * the wallet's decoy selection sanity check on aged test chains.
 */
async function sweepOutputsIndividually(wallet: moneroTs.MoneroWallet, relay: boolean): Promise<moneroTs.MoneroTxWallet[]> {
  const txs: moneroTs.MoneroTxWallet[] = [];
  const address = await wallet.getPrimaryAddress();
  for (const output of await wallet.getOutputs({isSpent: false, isFrozen: false, txQuery: {isLocked: false}})) {
    try {
      txs.push(await wallet.sweepOutput({address: address, keyImage: output.getKeyImage().getHex(), relay: relay}));
    } catch (err: any) {
      HavenoUtils.log(1, "Error sweeping output (skipping): " + err.message); // e.g. output too small to cover fee
    }
  }
  return txs;
}
...
function getValidFormInputAux(form: PaymentAccountForm, fieldId: PaymentAccountFormField.FieldId, havenod: HavenoClient): string {
  ...
  switch (fieldId) {
    ...
    case PaymentAccountFormField.FieldId.ACCOUNT_NR:
      ...
      if (form.getId() === PaymentAccountForm.FormId.RAAST) return "03001234567"; // 11-digit mobile number
      if (form.getId() === PaymentAccountForm.FormId.PAYNOW) return "91234567"; // 8-digit PayNow mobile number
    ...
    case PaymentAccountFormField.FieldId.MOBILE_NR:
      if (form.getId() === PaymentAccountForm.FormId.SWISH) return "+46701234567"; // SE mobile number in international format
...
function testPaymentAccount(account: PaymentAccount, form: PaymentAccountForm) {
    ...
    switch (form.getId()) {
      ...
      case PaymentAccountForm.FormId.TRANSFERWISE:
        ...
        expect(account.getPaymentAccountPayload()!.getTransferwiseAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        break;
      ...
      case PaymentAccountForm.FormId.HAL_CASH:
        ...
        break;
      case PaymentAccountForm.FormId.MONESE:
        expect(account.getPaymentAccountPayload()!.getMoneseAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getMoneseAccountPayload()!.getMobileNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.MOBILE_NR).getValue());
        break;
      case PaymentAccountForm.FormId.MONEY_BEAM:
    ...
    case PaymentAccountForm.FormId.BIZUM:
        ...
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("EUR");
        break;
    case PaymentAccountForm.FormId.BLIK:
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getBlikAccountPayload()!.getExtraInfo()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.EXTRA_INFO).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("PLN");
        break;
    case PaymentAccountForm.FormId.VIPPS_MOBILEPAY:
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getVippsMobilePayAccountPayload()!.getMobileNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.MOBILE_NR).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getVippsMobilePayAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
    ...
    case PaymentAccountForm.FormId.PAGO_MOVIL:
        ...
        break;
    case PaymentAccountForm.FormId.QRIS:
        expect(account.getPaymentAccountPayload()!.getQrisAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getQrisAccountPayload()!.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
    ...
    case PaymentAccountForm.FormId.PAPARA:
        ...
        break;
    case PaymentAccountForm.FormId.RAAST:
        expect(account.getPaymentAccountPayload()!.getRaastAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getRaastAccountPayload()!.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
        expect(account.getPaymentAccountPayload()!.getRaastAccountPayload()!.getBankName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.BANK_NAME).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("PKR");
        break;
    case PaymentAccountForm.FormId.PAYNOW:
        expect(account.getPaymentAccountPayload()!.getPaynowAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getPaynowAccountPayload()!.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
    ...
    case PaymentAccountForm.FormId.WERO:
        ...
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("EUR");
        break;
    case PaymentAccountForm.FormId.FPS:
        expect(account.getPaymentAccountPayload()!.getFpsAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getFpsAccountPayload()!.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
        expect(account.getPaymentAccountPayload()!.getFpsAccountPayload()!.getBankName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.BANK_NAME).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("HKD");
        break;
    case PaymentAccountForm.FormId.DUITNOW:
        expect(account.getPaymentAccountPayload()!.getDuitnowAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getDuitnowAccountPayload()!.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
        expect(account.getPaymentAccountPayload()!.getDuitnowAccountPayload()!.getBankName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.BANK_NAME).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("MYR");
        break;
    case PaymentAccountForm.FormId.MB_WAY:
        expect(account.getPaymentAccountPayload()!.getMbWayAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getMbWayAccountPayload()!.getMobileNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.MOBILE_NR).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("EUR");
        break;
    case PaymentAccountForm.FormId.PAYPAY:
        expect(account.getPaymentAccountPayload()!.getPaypayAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getPaypayAccountPayload()!.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
    ...
    case PaymentAccountForm.FormId.MIR:
        ...
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("RUB");
        break;
    case PaymentAccountForm.FormId.YOOMONEY:
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getYoomoneyAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getYoomoneyAccountPayload()!.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("RUB");
        break;
    case PaymentAccountForm.FormId.UKRAINE_CARD:
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getUkraineCardAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
    ...
    case PaymentAccountForm.FormId.SATISPAY:
        ...
        expect(account.getTradeCurrenciesList()[0].getCode()).toEqual("EUR");
        break;
    case PaymentAccountForm.FormId.GCASH:
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getGcashAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getGcashAccountPayload()!.getMobileNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.MOBILE_NR).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
    ...
    case PaymentAccountForm.FormId.MPESA:
        ...
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        break;
    case PaymentAccountForm.FormId.MERCADO_PAGO:
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getMercadoPagoAccountPayload()!.getHolderName()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.HOLDER_NAME).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getMercadoPagoAccountPayload()!.getEmailOrMobileNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.EMAIL_OR_MOBILE_NR).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getCountryCode()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.COUNTRY).getValue());
        expect(account.getTradeCurrenciesList().length).toEqual(1);
        break;
    ...
    case PaymentAccountForm.FormId.WESTERN_UNION:
        ...
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getWesternUnionAccountPayload()!.getEmail()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.EMAIL).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getWesternUnionAccountPayload()!.getCity()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.CITY).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getWesternUnionAccountPayload()!.getState()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.STATE).getValue());
        expect(account.getTradeCurrenciesList().map(currency => currency.getCode()).join(",")).toEqual(getFormField(form, PaymentAccountFormField.FieldId.TRADE_CURRENCIES).getValue());
        break;
    ...
    case PaymentAccountForm.FormId.SAME_BANK:
        ...
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getBankAccountPayload()?.getBranchId()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.BRANCH_ID).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getBankAccountPayload()?.getAccountNr()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_NR).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getBankAccountPayload()?.getNationalAccountId()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.NATIONAL_ACCOUNT_ID).getValue());
        expect(account.getPaymentAccountPayload()!.getCountryBasedPaymentAccountPayload()!.getBankAccountPayload()?.getAccountType()).toEqual(getFormField(form, PaymentAccountFormField.FieldId.ACCOUNT_TYPE).getValue());
